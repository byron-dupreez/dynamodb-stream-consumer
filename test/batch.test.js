...
const generateDynamoMD5s = identify.generateDynamoMD5s;
const resolveDynamoEventIdAndSeqNos = identify.resolveDynamoEventIdAndSeqNos;
const resolveDynamoMessageIdsAndSeqNos = identify.resolveDynamoMessageIdsAndSeqNos;
...
const eventSourceARN = samples.sampleDynamoDBEventSourceArn(awsRegion, 'TEST_Table_DEV', '2017-03-08T23:59:58.102Z');
...
function createContext(streamType, sequencingRequired, sequencingPerKey, idPropertyNames, keyPropertyNames, seqNoPropertyNames, event, awsContext) {
  ...
  const context = {
    streamProcessing: {
      ...
      generateMD5s: generateDynamoMD5s,
      resolveEventIdAndSeqNos: resolveDynamoEventIdAndSeqNos,
      resolveMessageIdsAndSeqNos: resolveDynamoMessageIdsAndSeqNos,
  ...
  context.stage = 'dev';
  contexts.configureStandardContext(context, undefined, require('../default-dynamo-options.json'), event, awsContext, false);
...
test('new Batch & resolveBatchKey with 1 record', t => {
  ...
  const [, rec1] = sampleDynamoDBMessageAndRecord(eventID1, eventSeqNo1, eventSourceARN, '123', 456, 'ABC', 10, 1, 2, 3);
...
test('new Batch & resolveBatchKey with 1 unusable record & 2 messages', t => {
  ...
  const [msg2, rec2] = sampleDynamoDBMessageAndRecord(eventID2, seqNo2, eventSourceARN, '123', '456', 'ABC', 10, '2.5', '200', '20000000000000000000002');

  const eventID3 = 'E003';
  const seqNo3 = 10003;
  const [msg3, rec3] = sampleDynamoDBMessageAndRecord(eventID3, seqNo3, eventSourceARN, '456', '789', 'ABC', 10, 3.3, 300, '30000000000000000000003');

  const expectedKey = {
    streamConsumerId: 'D|TEST_Table_DEV/2017-03-08T23:59:58.102|my-function:dev',
    shardOrEventID: 'E|E002'
  };
...
test(`reviveTasks with 0 messages and 2 unusable records AND sequencingRequired ${sequencingRequired}`, t => {
  ...
  const eventID2 = 'E002';
  const seqNo2 = '10000000000000002';
  const [, rec2] = sampleDynamoDBMessageAndRecord(eventID2, seqNo2, eventSourceARN, '123', '456', 'ABC', 10, 1, 2, 3);
...
test('reviveTasks with 2 messages and 0 unusable records AND with sequencingRequired true', t => {
  ...
  const eventID2 = 'E002';
  const seqNo2 = '10000000000000002';
  const [msg2, rec2] = sampleDynamoDBMessageAndRecord(eventID2, seqNo2, eventSourceARN, '123', '456', 'ABC', 10, 1, 2, 3);

  const eventID3 = 'E003';
  const seqNo3 = '10000000000000003';
  const [msg3, rec3] = sampleDynamoDBMessageAndRecord(eventID3, seqNo3, eventSourceARN, '456', '789', 'DEF', 11, 4, 5, 6);
...
test(`reviveTasks with with 2 ok messages and 1 rejected message AND sequencingRequired ${sequencingRequired}`, t => {
  ...
  const eventID1 = 'E001';
  const eventSeqNo1 = '10000000000000001';
  const [msg1, rec1] = sampleDynamoDBMessageAndRecord(eventID1, eventSeqNo1, eventSourceARN, '123', '455', 'ABC', 10, 1, 2, 3);

  const eventID2 = 'E002';
  const seqNo2 = '10000000000000002';
  const [msg2, rec2] = sampleDynamoDBMessageAndRecord(eventID2, seqNo2, eventSourceARN, '123', '456', 'ABC', 10, 4, 5, 6);

  const eventID3 = 'E003';
  const seqNo3 = '10000000000000003';
  const [msg3, rec3] = sampleDynamoDBMessageAndRecord(eventID3, seqNo3, eventSourceARN, '456', '789', 'DEF', 11, 7, 8, 9);
...
test(`abandonDeadProcessingTasks`, t => {
  ...
  const eventID1 = 'E001';
  const eventSeqNo1 = '10000000000000001';
  const [msg1, rec1] = sampleDynamoDBMessageAndRecord(eventID1, eventSeqNo1, eventSourceARN, '123', '456', 'ABC', 10, 1, 2, 3);

  const rec2 = {bad: 'apple'};

  const eventID3 = 'E003';
  const seqNo3 = '10000000000000003';
  const [msg3, rec3] = sampleDynamoDBMessageAndRecord(eventID3, seqNo3, eventSourceARN, '123', '456', 'ABC', 10, 1, 2, 4);
